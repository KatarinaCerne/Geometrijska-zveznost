...
\begin{definicija}\label{def2}
  ...
  krivulje $\ddd{C}$}, če za vsako točko $b_0=\ddd{C}(y_0)$ obstaja lokalno regularna $C^n$ reparametrizacijska funkcija $f(x,y)=(u(x,y),v(x,y))$, da je $f(x_0,y)=(u_0,y)$ za vsak $y \in I_0$ in da velja
  \begin{equation}\label{eq0}
  \frac{\partial^{m+k}}{\partial x^m \partial y^k}\ddd{R}\Bigr|_{\substack{(x_0,y)}}=\frac{\partial^{m+k}}{\partial x^m \partial y^k}(\ddd{S}\circ f)\Bigr|_{\substack{(x_0,y)}} \textrm{ za } m+k=1,\ldots,n,
  \end{equation}
...
\begin{primer}
Naj bodo parametrizaciji ploskev $\ddd{R}$ in $\ddd{S}$, krivulja $\ddd{C}$ in 
reparametrizacijska 
...
$$\frac{\partial^{k}}{\partial x^k}\ddd{R}\Bigr|_{\substack{(x_0,y)}}=\frac{\partial^{k}}{\partial x^k}(\ddd{S}\circ f)\Bigr|_{\substack{(x_0,y)}} \textrm{ za } k=0,1,2.$$
...
\frac{\partial^k \ddd{R}}{\partial x^k}\bigg|_{\substack{C}}=\sum_{i=1}^k \sum_{|m_i|=k}A_{m_i}^k
...
\begin{proof}
  ...
  ujemanja ploskev $\ddd{R}$ in $\ddd{S}$ v krivulji $\ddd{C}$ sledi tudi enakost \ref{eq0}. Pokazati moramo 
  ...
  \todo{ali so take funkcije $C^n$?}

  Ker je stik ploskev $\ddd{R}$ in $\ddd{S}$ $G^n$-zvezen, do definiciji \ref{def2} obstaja 
...
Zgornja enačba nam pove, da so parcialni odvodi $\ddd{R}_x(x_0,y)$, $\ddd{S}_u(u_0,y)$ in 
$\ddd{S}_v(u_0,y)$ v vsaki točki $y$ linearno odvisni. Torej so v vsaki točki $y$ del 
iste tangentne ravnine na krivuljo $\ddd{C}$. Tangentna ravnina na ploskev $\ddd{R}$ 
...
Upoštevamo lahko, da je $\ddd{R}_y(x_0,y)=\ddd{S}_v(u_0,y)$. Dobimo:
...
krivulji $\ddd{C}$. 
Enačba \ref{eq30} pove, da sta normali na ploskvi $\ddd{R}$ in $\ddd{S}$ na njunu stični 
...
Ker parcialni odvodi $\ddd{R}_x(x_0,y)$, $\ddd{S}_u(u_0,y)$ in $\ddd{S}_v(u_0,y)$ ležijo na isti 
tangentni ravnini, velja tudi:
$$\det(\ddd{R}_x(x_0,y), \ddd{S}_u(u_0,y), \ddd{S}_v(u_0,y))=0.$$

\todo{Torej obstajajo funkcije (povedati kakšne, iz kje kam?) $\lambda$, 
$\mu$ in $\gamma$, da velja:
$$\lambda(y)R_x(x_0,y)=\mu(y)S_u(u_0,y)+\gamma(y)S_v(u_0,y).$$}

\todo{Če predpostavimo, da sta ploskvi $R$ in $S$ polinomski, lahko tudi za $\lambda$, $\mu$ in $\gamma$ 
...
imenovano Bézierove ploskve, in s pogoji, ki morajo veljati zanje, da je stik 
med njimi geometrijsko zvezen. 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Konstrukcija geometrijsko zveznih Bezierovih ploskev 
...
Bézierove krivulje in ploskve podajamo s pomočjo Bernsteinovih polinomov, $i$-ti 
Bernseinov polinom je definiran kot  

$$B_i^n(t)={n \choose i}t^i (1-t)^{n-i}\textrm{, } t\in[0,1].$$

Bernsteinovi polinomi sestavljajo bazo prostora polinomov \\
$\mathbb{P}_n=Lin\{1,t,t^2,\ldots,t^n\}$. 
Bazo prostora polinomov dveh spremenljivk 
$\mathbb{P}_m \otimes \mathbb{P}_n = Lin\{u^iv^j, i=0,\ldots,m, j=0,\ldots,n\}$, 
torej tenzorskega produkta prostorov $\mathbb{P}_m$ in $\mathbb{P}_n$, pa sestavljajo 
produkti po dveh Bernsteinovih plinomov, kjer prvi pripada $\mathbb{P}_m$ in 
...
\begin{definicija}
  ...
  Točke $\mathbf{b}_{i,j}$ imenujemo \emph{kontrolne točke}, poligon, ki jih povezuje, 
  pa \emph{kontrolni poligon} ploskve $\ddd{B}_{m,n}$. 
\end{definicija}
...
\end{definicija}
Tudi za Bézierove krivulje velja, da interpolirajo prvo in zadnjo kontrolno točko. 
...
\subsection{$C^n$-zveznost med dvema Bézierovima ploskvama 
iz tenzorskega produkta}\label{cn.zv.}

Najprej si oglejmo, kakšni pogoji morajo veljati za dve Bézierovi ploskvi, da 
je njun stik $C^n$-zvezen. Dobljeni rezultat bomo v nadaljevanju primerjali s 
pogoji za $G^n$-zveznost. 

Imejmo Bézierovi ploskvi 
$$\ddd{R}(u,v)=\sum_{i=0}^m\sum_{j=0}^n\ddd{r}_{i,j}B_i^m(\frac{u-u_1}{u_0-u_1})B_j^n(\frac{v-v_0}{v_1-v_0})\textrm{, }
u\in[u_0,u_1]\textrm{, }v\in[v_0,v_1]$$
in 
$$\ddd{S}(u,v)=\sum_{i=0}^m\sum_{j=0}^n\ddd{s}_{i,j}B_i^m(\frac{u-u_1}{u_2-u_1})B_j^n(\frac{v-v_0}{v_1-v_0})\textrm{, }
...
$j=0,\ldots,n$. S tem dosežemo $C^1$-zveznost. Da se bosta ploskvi stikali s 
$C^r$-zveznostjo, se morajo ujemati njuni odvodi vzdolž krivulje $\ddd{C}$ 
do odvoda stopnje $r$. Zaradi stikanja v krivulji $\ddd{C}$ je dovolj obravnavati le 
odvode po spremenljivki $u$. Veljati mora:
$$\frac{\partial^k}{\partial u}\ddd{R}(u,v)|_{u=u_0}=\frac{\partial^k}{\partial u}\ddd{S}(u,v)|_{u=u_0},
\textrm{ }k=1,\ldots,r.$$
Uporabimo enačbo \ref{eq28} za odvod Bézierove ploskve iz tenzorskega produkta: 
\begin{align*}
\frac{n!}{(n-k)!}&\frac{1}{(u_0-u_1)^k}\sum_{i=0}^{m-k}\sum_{j=0}^n\Delta^{k,0}
\ddd{r}_{i,j}B_i^{m-k}(\frac{u-u_1}{u_0-u_1})B_i^n(\frac{v-v_0}{v_1-v_0})|_{u=u_1}=\\
&=\frac{n!}{(n-k)!}\frac{1}{(u_2-u_1)^k}\sum_{i=0}^{m-k}\sum_{j=0}^n\Delta^{k,0}
\ddd{s}_{i,j}B_i^{m-k}(\frac{u-u_1}{u_2-u_1})B_i^n(\frac{v-v_0}{v_1-v_0})|_{u=u_1},\\
k=1,\ldots,r
\end{align*}
Sledi
\begin{align*}
&\frac{1}{(u_0-u_1)^k}\sum_{j=0}^n\Delta^{k,0}
\ddd{r}_{0,j}B_i^n(\frac{v-v_0}{v_1-v_0})=
\frac{1}{(u_2-u_1)^k}\sum_{j=0}^n\Delta^{k,0}
\ddd{s}_{0,j}B_i^n(\frac{v-v_0}{v_1-v_0}),\\
&k=1,\ldots,r.
\end{align*}
...
odražajo na Bézierovih ploskvah iz tenzorskega produkta. 

Ker so Bézierove ploskve polinomske, se lahko pri izbiri stičnih funkcij 
...
$$N_r(y)=\bigg(\frac{\partial \textbf{R}}{\partial x}\times \frac{\partial \textbf{R}}{\partial y}\bigg)\bigg|_{\textbf{C}}\neq 0$$
$$N_s(y)=\bigg(\frac{\partial \textbf{S}}{\partial u}\times \frac{\partial \textbf{S}}{\partial v}\bigg)\bigg|_{\textbf{C}}\neq 0$$

O pogojih za geometrijsko zveznost teh dveh ploskev govori naslednji izrek: 

\begin{izrek}\label{izrek2}
  Naj bosta $\textbf{R}$ in $\textbf{S}$ zgoraj definirani Bézierovi 
  ...
  $st(F_i)\leq (2i-1)n_r +in_s + (i-1)n_c -2i +2.$
\end{izrek}
...
\begin{proof}
  ...
  Preoblikujmo enačbo 
  \begin{align*}
    D^{2k-1}(y)\frac{\partial^k\textbf{S}}{\partial u^k}\bigg|_{\textbf{C}}(y) = &
    \sum_{i=0}^k \sum_{|m_i|=k} A_{m_i}^k \sum_{h=0}^i {i\choose h} D^{i-1}(y)E_{m_1}(v)
    \cdots E_{m_h}(y) \\
    &\cdot F_{m_{h+1}}(y)\cdots F_{m_i}(y) \frac{\partial^i \textbf{R}}{\partial x^h \partial y^{i-h}}(y).
  \end{align*}
  Če celotno enačbo delimo z $D^{2k-1}$ (predpostavka, da $D(y)E_1(y)\neq 0$ na $[0,1]$, zagotavlja neničelnost polinoma $D$ na $[0,1]$), 
  dobimo 
  \begin{equation}\label{eq5}
    \begin{split}
    \frac{\partial^k\textbf{S}}{\partial u^k}\bigg|_{\textbf{C}}(y) = &
  ...
  \begin{equation}\label{eq6}
    ...
    &\cdot \frac{F_{m_{h+1}}(y)}{D^{2m_{h+1}-1}(y)}\cdots \frac{F_{m_i}(y)}{D^{2m_i-1}(y)} \frac{\partial^i \textbf{R}}{\partial x^h \partial y^{i-h}}(y).
    \end{split}
  \end{equation}
  ...
  dokazali \todo{eno smer ekvivalence}
  ...
  pa manjša ali enaka $n_s$. \todo{dodatno razloži?}
  ...
  \ddd{W}_1 = \ddd{S}_{m+1}\times\frac{\partial \ddd{R}}{\partial y}|_{\ddd{C}} 
  \end{equation}
  in
  \begin{equation}\label{eq32} 
  \ddd{W}_2 = \frac{\partial \ddd{R}}{\partial x}|_{\ddd{C}}\times \ddd{S}_{m+1}
  \end{equation}
  ter kakor prej 
  ...
  Iz enačbe \ref{eq31} in definicije polinoma $E_{m+1}(y)$ je razvidno naslednje:
  \begin{align*}
  ...
  Podobno dobimo iz enačbe \ref{eq32} in definicije polinoma $F_{m+1}(y)$ oceno za 
  stopnjo polinoma $F_{m+1}(y)$:
  \begin{align*}
...
\begin{primer}\label{primer1}
  ...
  Če razpišemo oba parcialna odvoda oziroma se sklicujemo na enakost \ref{eq29}, 
...
Vstavimo polinome $D$, $E_1$ in $F_1$ v enačbo \ref{eq16} in dobimo:
\begin{align*}
...
$\sum_{j=0}^3\ddd{z}_jf_0{2 \choose j}v^j(1-v)^{3-j}$ člen pri $j=3$ enak 0 
in je zato \\$\sum_{j=0}^2\ddd{z}_jf_0{2 \choose j}v^j(1-v)^{3-j}=\sum_{j=0}^3\ddd{z}_jf_0{2 \choose j}v^j(1-v)^{3-j}$.
Podobno smo v zadnji vrstici upoštevali, da je v vsoti $\sum_{j=0}^3b_1\ddd{z}_{j-1}\frac{j}{3}B_j^3(v)$ 
...
Od tod dobimo pogoje za kontrolna vektorja $\ddd{q}_1$ in $\ddd{q}_2$:
...
Parametri so določena z enačbama \ref{eq18} in \ref{eq19}. 
\todo{kako se jih izračuna- normala?} 
%eden pa je prost, z njim lahko spreminjamo obliko končne ploskve.
\todo{?}Če imamo  
$e_0=-1$, $f_0=0$ in $f_1=0$, dobimo enak rezultat kot v primeru \ref{primer1}, 
v katerem smo iskali pogoje za $C^1$-zveznost. Z drugačno izbiro parametrov pa 
lahko dosežemo poljuben kot med vektorjema $q_{1,1}$ in $p_{1,1}$, biti morata samo 
del iste ravnine. \todo{?}  
...
\begin{primer}\label{primer3}
  ...
  Stopnja polinoma $D(v)$ naj bo znova 0, stopnja $F_1(v)$ pa 1, med tem ko naj bo 
  polinom $E_1(v)$ 
  stopnje 1. Da bomo v tem primeru na obeh straneh enačbe \ref{eq15} dobili 
...
Enačba \ref{eq15} se torej v tem primeru preoblikuje tako:
\begin{align}\label{eq16}
...
Vstavimo polinoma v enačbo \ref{eq16} in na podoben način kot v primeru \ref{primer2} dobimo:
\begin{align*}
...
%Dva izmed njih sta določena z enačbama \ref{eq18} in \ref{eq19}, druga dva pa sta prosta. 
%S tem dobimo nove možnosti za obliko dobljene ploskve.
\todo{parametri e1, e2, f1, f2 določeni iz enačb za robne kontrolne točke?}
...
prosti. Prosta je le še ena izmed njiju, druga pa je določena z enačbo \ref{eq17}.
...
Naj bosta $\ddd{R}$ in $\ddd{S}$ bikvintični Bézierovi ploskvi iz tenzorskega produkta: 
...
veljala enakost \ref{eq33} ter 
\begin{equation}\label{eq34}
  \begin{split}
\frac{\partial^2\ddd{S}}{\partial u^2}|_{u=0}(v)=&\alpha_2(v)\frac{\partial \ddd{R}}{\partial u}|_{u=0}(v)
+\beta_2(v)\frac{\partial\ddd{R}}{\partial v}|_{u=0}(v)+\alpha_1^2(v)\frac{\partial^2\ddd{R}}{\partial u^2}|_{u=0}(v)+\\
&+2\alpha_1(v)\beta_1(v)\frac{\partial^2\ddd{R}}{\partial u \partial v}|_{u=0}(v)+\beta_1^2(v)
\frac{\partial^2\ddd{R}}{\partial v^2}|_{u=0}(v).
  \end{split}
\end{equation}
Ker gre znova za Bézierove ploskve iz tenzorskega produkta, lahko uporabimo 
...
velja enakost \ref{eq14} ter 
\begin{equation}\label{eq35}
  \begin{split}
D^3(v)\frac{\partial^2\ddd{S}}{\partial u^2}|_{u=0}(v)=&E_2(v)\frac{\partial \ddd{R}}{\partial u}|_{u=0}(v)
+F_2(v)\frac{\partial\ddd{R}}{\partial v}|_{u=0}(v)+D(v)E_1^2(v)\frac{\partial^2\ddd{R}}{\partial u^2}|_{u=0}(v)+\\
&+2D(v)E_1(v)F_1(v)\frac{\partial^2\ddd{R}}{\partial u \partial v}|_{u=0}(v)+D(v)F_1^2(v)
\frac{\partial^2\ddd{R}}{\partial v^2}|_{u=0}(v).
  \end{split}
\end{equation}
...
Sklicujmo se na enakost \ref{eq28} in dobimo naslednje rezultate za parcialne odvode 
parametrizacij ploskev $\ddd{R}$ in $\ddd{S}$ v $u=0$: 

\begin{align*}
&\frac{\partial \ddd{R}}{\partial u}|_{u=0}=5\sum_{j=0}^5\ddd{p}_{1,j}B_j^5(v)&&
\frac{\partial \ddd{R}}{\partial v}|_{u=0}=5\sum_{j=0}^4\ddd{z}_jB_j^4(v)\\
&\frac{\partial^2\ddd{S}}{\partial u^2}|_{u=0}=5\cdot 4\sum_{j=0}^5\ddd{q}_{2,j}B_j^5(v)&&
\frac{\partial^2\ddd{R}}{\partial u^2}|_{u=0}=5\cdot 4\sum_{j=0}^5\ddd{p}_{2,j}B_j^5(v)\\
&\frac{\partial^2\ddd{R}}{\partial v^2}|_{u=0}=5\cdot 4\sum_{j=0}^3\ddd{s}_jB_j^3(v)&&
\frac{\partial^2\ddd{R}}{\partial u \partial v}|_{u=0}=5\cdot 5\sum_{j=0}^4\ddd{v}_jB_j^4(v).
\end{align*}

Dobljene izraze za odvode vstavimo v enačbi \ref{eq14} in \ref{eq35}. Oglejmo si, kakšne 
...
vektorji, ki nastopajo v zgornjih enačbah(\ref{eq36}-\ref{eq39}), razen vektorjev $\ddd{v}_0$ in $\ddd{v}_4$ 
znani. Enačbi \ref{eq36} in \ref{eq38} enolično določata vrednosti parametrov 
$e_{10}$, $f_{10}$, $e_{11}$ in $f_{11}$. \todo{glej primer g1} Proste parametre oziroma 
kontrolne točke nam bosta prinesli le enačbi \ref{eq37} in \ref{eq39}. 

Najprej si oglejmo enačbo \ref{eq37} in obravnavajmo dva primera. V prvem primeru 
...
le dva še neznana parametra $e_{20}$ in $f_{20}$, ki ju lahko določimo iz enačbe. 
\todo{glej primer g1} \todo{skalarni produkt z normalo?} V tem primeru nam enačba 
\ref{eq27} ne da nobenih prostih parametrov ali kontrolnih točk. Če pa je $f_{10}\neq 0$, 
...
enačbe \ref{eq37}, projekcija vektorja $\ddd{v}_0$ na tangentno ravnino na ploskev v 
točki $\ddd{P}_{0,0}$ pa je prosta. 
Druga možnost pa je, da sta parametra $e_{20}$ in $f_{20}$ prosta, vektor $\ddd{v}_0$ 
pa je določen z enačbo \ref{eq37}. 

Na enak način lahko analiziramo enačbo \ref{eq39}. V prvem primeru, kjer je $f_{11}=0$, 
člen, ki vsebuje vektor $\ddd{v}_4$, izgine, parametra $e_{21}$ in $f_{21}$ pa sta 
določena z enačbo \ref{eq39}. Če je $f_{11}\neq 0$ pa znova dobimo dve situaciji. Lahko 
določimo projekcijo vektorja $\ddd{v}_4$ na normalo $\ddd{n}_2$ na ploskev v točki 
$\ddd{P}_{0,5}$ ter parametra $e_{21}$ in $f_{2,1}$ iz enačbe \ref{eq39}, projekcija 
...
odvisni od stopenj Bézierjevih krivulj tako v enačbi \ref{eq14} kot v enačbi \ref{eq35}. 
...
enačbah \ref{eq14} in \ref{eq35} ujemale, moramo znižati stopnji parcialnih odvodov 
...
$\ddd{\tilde{p}}_i$ s pomočjo enačb za višanje stopnje Bézierjeve krivulje \todo{sklic} 
na naslednji način:
...
Iz enačb \ref{eq40} lahko izrazimo še vektorja $\ddd{\tilde{p}}_1$ in $\ddd{\tilde{p}}_2$ kot 
...
Enako lahko s pomočjo enačb za višanje stopnje \todo{sklic} izrazimo kontrolne vektorje 
...
uporabiti enačbi \ref{eq14} in \ref{eq35}, ki predstavljata pogoja za $G^1$ in $G^2$-zveznost. 
Ko v enačbo \ref{eq14} vstavimo polinoma $E_1(v)$ in $F_1(v)$, preoblikujemo dobljeno 
enačbo, da na obeh straneh dobimo Bézierovo krivuljo stopnje 5 ter primerjamo koeficiente 
...
Ko enak postopek ponovimo še z enačbo \ref{eq35}, pa dobimo:
...
\subsection{Trikotne Bézierove ploskve}\label{trikotne1}

\todo{povej, kje je možno najti dokaze trditev v tem podpoglavju}

Trikotne Bézierove ploskve so tip Bézierovih ploskev s parametrizacijo, definirano 
...
in $\ddd{p}_3=(x_3,y_3)$, oziroma $T=<\ddd{p}_1,\ddd{p}_2,\ddd{p}_3>$. Velja, 
...
na trikotnik $T$. Krajše to zapišemo kot $\ddd{u}=Bar(\ddd{p},T)$.

Trikotne Bezierove ploskve definiramo na naslednji način. 
\begin{definicija}
  \emph{Trikotna Bézierova ploskev stopnje n} je podana s parametrizacijo
  $$\ddd{R}_n(\ddd{p})=\sum_{i+j+k=n}\ddd{b}_{i,j,k}B_{i,j,k}^n(\ddd{u}),$$
  kjer so $\ddd{b}_{i,j,k}$, $i+j+k=n$, kontrolne točke te ploskve in 
  $\ddd{u}=Bar(\ddd{p};T)$. Trikotnik $T$ je domena te parametrizacije. Z 
...
V nadaljevanju si bomo ogledali še definicijo smernega odvoda trikotne Bézierove 
ploskve in pogoje za $C^n$-zveznost med dvema trikotnima Bézierovima ploskvama, 
...
$\ddd{b}_{i,j,k}=\ddd{b}[\ddd{e}_1^{<i>},\ddd{e}_2^{<j>},\ddd{e}_3^{<k>}]$.

S pomočjo trikotniškega razcveta lahko izrazimo kontrolne točke ploskve glede na 
različne trikotnike v domeni. Imejmo trikotnika $T_1=<\ddd{p}_1,\ddd{p}_2,\ddd{p}_3>$ in 
...
$c_{i,j,k} = \ddd{b}[\ddd{s}_1^{<i>},\ddd{s}_2^{<j>},\ddd{s}_3^{<k>}]$, $i+j+k=n$. 
Tu s $\ddd{s}_1^{<i>}$ označujemo $i$-kratno ponovitev $\ddd{s}_1$.

Če imamo domeno, sestavljeno iz dveh trikotnikov, ki se na enem robu stikata, 
torej če imamo trikotnika $T_1=<\ddd{p}_1,\ddd{p}_2,\ddd{p}_3>$ in 
$T_2 = <\ddd{p}_2,\ddd{p}_3,\ddd{p}_4>$, ter ploskev, podano s kontrolnimi točkami 
glede na trikotnik $T_1$: $b_{i,j,k},$ $i+j+k=n$,   
lahko kontrolne točke ploskve  
glede na trikotnik $T_2$ zapišemo tako: 
$\ddd{c}_{i,j,k}=\ddd{b}[\ddd{e}_1^{<i>},\ddd{e}_2^{<j>},\mathbf{\alpha}^{<k>};T]$. 
...
Oglejmo si še formulo za smerni odvod parametrizacije trikotne Bézierove ploskve. 
...
trikotnima domenama $T_1=<\ddd{p_1},\ddd{p_2},\ddd{p}_3>$ in $T_2=<\ddd{p}_4,\ddd{p}_2,\ddd{p}_3>$:
$$\ddd{R}(\ddd{p})=\sum_{i+j+k=n}\ddd{r}_{i,j,k}B_{i,j,k}^n(\ddd{u});
\textrm{ }\ddd{u}=Bar(\ddd{p};T_1),$$
$$\ddd{S}(\ddd{p})=\sum_{i+j+k=n}\ddd{s}_{i,j,k}B_{i,j,k}^n(\ddd{v});
\textrm{ }\ddd{v}=Bar(\ddd{p};T_2).$$
Zapišimo točki $\ddd{u}$ in $\ddd{v}$ kot $\ddd{u}=(u_1,u_2,u_3)$ in $\ddd{v}=(v_1,v_2,v_3)$. 
Ploskvi se stikata na robu nad daljico $\bar{\ddd{p}_2\ddd{p}_3}$, oziroma na 
...
Naj bodo $\ddd{\mu}=(\mu_1,\mu_2,\mu_3)=Bar(\ddd{d};T_1)=\ddd{\alpha}-\ddd{e}_2$ baricentrične 
koordinate vektorja $\ddd{d}$ glede na trikotnik $T_1$, kjer je $\ddd{\alpha}=
Bar(\ddd{p}_4;T_1)$, in 
$\ddd{\eta}=(\eta_1,\eta_2,\eta_3)=Bar(\ddd{d};T_2)=\ddd{e}_1-\ddd{e}_2$ baricentrične 
...
ter dobljeno vstavimo v enačbo \ref{eq20}, ob tem pa upoštevamo še, da se 
...
\end{align}
za $j+k=n-1$. 
Enačba \ref{eq21} predstavlja pogoj, ki mora veljati za kontrolne točke obeh 
...
%Alternativni način za izpeljavo pogojev \ref{eq21}
...
$\ddd{\alpha}=(\alpha_1,\alpha_2,\alpha_3)=Bar(\ddd{p}_4;T_1)$.
...
Da bo stik obeh ploskev še $C^1$ zvezen mora po enačbi \ref{eq21} veljati:
\begin{align*}
...
$<\ddd{r}_{1,0,2},\ddd{r}_{0,1,2},\ddd{r}_{0,0,3}>$ in 
$\ddd{s}_{0,1,2},\ddd{s}_{0,0,3},\ddd{s}_{1,0,2}>$ morata biti afini sliki 
domenskih trikotnikov $T_1$ in $T_2$. Enako mora veljati tudi za trikotnika 
$<\ddd{r}_{1,1,1},\ddd{r}_{0,2,1},\ddd{r}_{0,1,2}>$ in 
$<\ddd{s}_{0,2,1},\ddd{s}_{0,1,2},\ddd{s}_{1,1,1}>$ ter trikotnika 
$<\ddd{r}_{1,2,0},\ddd{r}_{0,3,0},\ddd{r}_{0,2,1}>$ in 
$<\ddd{s}_{0,3,0},\ddd{s}_{0,2,1},\ddd{s}_{1,2,0}>$. 
\end{primer}
...
Imejmo trikotni ploskvi $\ddd{R}$ in $\ddd{S}$ kot v primeru \ref{primerT2}. 
...
$<\ddd{r}_{1,0,2},\ddd{r}_{0,1,2},\ddd{r}_{0,0,3}>$ in 
$\ddd{s}_{0,1,2},\ddd{s}_{0,0,3},\ddd{s}_{1,0,2}>$ ter 
$<\ddd{r}_{1,2,0},\ddd{r}_{0,3,0},\ddd{r}_{0,2,1}>$ in 
$<\ddd{s}_{0,3,0},\ddd{s}_{0,2,1},\ddd{s}_{1,2,0}>$ 
nista afini sliki istega para domenskih trikotnikov. 
\end{primer}

V zgornjem primeru smo videli, da je pogoj $C^1$-zveznosti za trikotne Bézierove 
...
Imejmo dve trikotni Bézierovi ploskvi prve stopnje
$$\ddd{R}(\ddd{p})=\ddd{r}_{1,0,0}u+\ddd{r}_{0,1,0}v+\ddd{r}_{0,0,1}w; 
\textrm{ }(u,v,w)=Bar(\ddd{p};T_1)$$
in
$$\ddd{S}(\ddd{p})=\ddd{s}_{1,0,0}\tilde{u}+\ddd{s}_{0,1,0}\tilde{v}+
\ddd{s}_{0,0,1}\tilde{w}; \textrm{ }(\tilde{u},\tilde{v},\tilde{w})=Bar(\ddd{p};T_2).$$
...
Trikotnika $T_1$ in $T_2$ definirajmo kot $T_1=<(0,1),(0,0),(1,0)>$ in 
$T_2=<(0,1),(0,0),(1,0)>$. 
...
Naj bo $\ddd{\alpha}=Bar(\ddd{s}_{1,0,0};T_1)$. Torej je 
$\ddd{\alpha}=(-1,2,0)$. Da bi bil stik ploskev $C^1$-zvezen, bi moralo biti 
zadoščeno enačbi \ref{eq21}, torej bi moralo veljati 
...
\textrm{ }\ddd{\tilde{u}}=Bar(\ddd{p};T_2).$$
Naj bo $\ddd{u}=(u,v,w)$ in $\ddd{\tilde{u}}=(\tilde{u},\tilde{v},\tilde{w})$. 
Ploskvi se stikata na robu nad daljico $\bar{\ddd{p}_2\ddd{p}_3}$, oziroma na 
...
$\bar{\ddd{r}_{0,1,0}^{n-1}(v)\ddd{r}_{0,0,1}^{n-1}(v)}$ in 
$\bar{\ddd{r}_{1,0,0}^{n-1}(v)\ddd{s}_{1,0,0}^{n-1}(v)}$ sekati za vsako 
...
Najprej si oglejmo, kakšne pogoje nam da enačba \ref{eq22} za robne kontrolne točke. 
Vstavimo v enačbo \ref{eq22} vrednosti $v=0$ in $v=1$. 
Pri vrednosti $v=0$ dobimo: 
...
Polinoma vstavimo v enačbo \ref{eq22}. Najprej si oglejmo izraz \\
...
Dobljeno vstavimo v enačbo \ref{eq22} in primerjajmo člene ob baznih polinomih 
...
Pogoji, ki jih dobimo z enačbo \ref{eq23} za kontrolne točke ploskev $\ddd{R}$ 
...
\begin{primer}\label{primerT4}
  ...
  \textrm{ }\ddd{\tilde{u}}=Bar(\ddd{p};T_2),$$
  kjer je $\ddd{u}=(u,v,w)$ in $\ddd{\tilde{u}}=(\tilde{u},\tilde{v},\tilde{w})$. 
  Ploskvi se ponovno stikata na robu nad daljico $\bar{\ddd{p}_2\ddd{p}_3}$, oziroma na 
  robu, določenem z $u=\tilde{u}=0$. 
  ...
  %in $\ddd{s}_{1,1,1}$, nam enačbi \ref{eq24} in \ref{eq25} določata dva izmed 

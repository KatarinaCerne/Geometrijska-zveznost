...
oziroma (po izreku \todo{sklic}) natanko tedaj ko obstajajo polinomi $D(v)$, $E_1(v)$ 
...
\todo{ker gre za bezierove ploskve, sta alfa1 in beta1 racionalni funkciji. 
kako naj to razložim zakaj?}
...
$$D(v)\sum_{j=0}^3(\ddd{P}_{1,j}-\ddd{P}_{0,j})B_j^3(v) = E_1(v)\sum_{j=0}^3(\ddd{Q}_{1,j}-\ddd{Q}_{0,j})B_j^3(v) + 
...
evalvirajmo zgornjo enačbo \todo{sklic} v vrednostih paramatera $v=0$ in $v=1$ 
\todo{grdo}.
\nevem{kako nam ti pogoji določajo robne vektorje}
\todo{ spet se lahko skliciješ na tisto lastnost}
...
je vse že natančno določeno ?} \nevem{kontrolne točke nikoli ne bodo enolično določene,
kar hočemo narediti je samo videti, kakšna razmerja veljajo, kaj je določeno, kaj je prosto,
kaj je odvisno od česa. po drugi strani pa je pri C zveznosti vse do konca določeno?}
...
Zaradi enostavnosti \nevem{NE} naj bosta \todo{koeficientni} funkciji $\alpha_1$ in $\beta_1$ polinoma. 

...
\begin{definicija}
  ...
  za $y\in I_0$, kjer je $I_0$ neka okolica $y_0$. %bolj natančno, kaj je ta okolica
\end{definicija}
\todo{napiši R:omega1 pod r2 v r3, S:omega2pod r2 v r3 (wut?)}
\todo{povej, da so odvodi vzdolž y=v enaki (dodati sliko?), zato je dovolj, da gledamo samo odvode vzdolž x in u}
...
\todo{zdaj si pogledamo ekvivalentno/alternativno definicijo G zveznosti}
\todo{definirajmo reparametrizacijo}
...
\begin{flalign*}
  ...
  \frac{\partial^i v}{\partial x^i}(x_0,y)&=\beta_i(y)\textrm{ za }i=1, \ldots, k.
\end{flalign*}

Regularnost reparametrizacije:
\begin{flalign*}
  &\frac{\partial f}{\partial x}\times \frac{\partial f}{\partial y} \neq 0&\\
...
Z uvedbo funkcij $\alpha_1, \ldots, \alpha_n$ in $\beta_1 \ldots, \beta_n$ pridemo do nekoliko drugačne definicije
...
\end{definicija}
\todo{povej, da to velja za vsak j=1,...,n;}
\todo{popravi za vrednosti funkcij (wut?);}
\todo{$\alpha_j$, $\beta_j$ stične funkcije;}
\todo{$\alpha_1(y)\neq 0$ in potrebno izbrati pravi predznak $\alpha_1$ - to še ugotovi in razloži}

\begin{izrek} \todo{?? ta izrek samo pove, da so definicije enakovredne. dokaz? je v bistvu celo poglavje}
...
\todo{nekaj v stilu: imamo ploskvi R, S, ki ustrezata definiciji. zanju veljajo naslednji pogoji:}
\begin{flalign*}
  &R(x_0,y)=S(u_0,y)&\\ \todo{povej da Ry = Sy vzdolž C!, zato nam ni treba tega pogoja posebej poudarjati}
  &R_x(x_0,y)=u_x(x_0,y)S_u(u_0,y)+v_x(x_0,y)S_v(u_0,y)\\ \todo{oziroma kot smo prej dobili...obstajata alfa1 in beta1, kjer alfa1 ni 0 in ustreznega predznaka}
  &R_x(x_0,y)=\alpha_1(y) S_u(u_0,y)+\beta_1(y)S_v(u_0,y)\\ 
  \todo{kako to geometrijsko interpretiramo: vidimo, da so Rx, Su, Sv linearno odvisni v vsaki točki y tj. v vsaki točki del iste tangentne ravnine;}
  \todo{na mestu najbrž kakšna slika}
\end{flalign*}
\todo{vidimo, da se na skupnem robu ujemata enotski normali;}
\todo{slika?;}
\todo{G1 zveznosti rečemo tudi zveznost tangentnih ravnin oz zveznost enotskih normal}

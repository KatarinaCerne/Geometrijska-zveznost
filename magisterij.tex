...
\begin{proof}
  V eno smer je dokaz očiten. Če predpostavimo, da sta ploskvi $R$ in $S$ na stiku $G^n$-zvezni, 
  izrek sledi neposredno iz definicije \ref{def2} in enačbe \ref{eq1}, če definiramo 
  $\alpha_i(y)=u_{x^i}(y)$ in $\beta_i(y)=v_{x^i}(y)$ za $i=1,\ldots,n$. 
...
(\ddd{Q}_{i+1,j}-\ddd{Q}_{i,j})B_i^2(u)B_j^3(v)|_{u=0}=
3\sum_{j=0}^2(\ddd{Q}_{1,j}-\ddd{Q}_{0,j})B_j^2(v).$$

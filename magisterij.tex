...
enotski normali:
$$\frac{\ddd{R}_x(x_0,y) \times \ddd{R}_y(x_0,y)}{||\ddd{R}_x(x_0,y) \times \ddd{R}_y(x_0,y)||}=
\frac{\alpha_1(y)\ddd{S}_u(u_0,y) \times \ddd{S}_v(u_0,y)}{||\alpha_1(y)\ddd{S}_u(u_0,y) \times \ddd{S}_v(u_0,y)||}.$$
...
tenzorskega produkta. \todo{povej, v katerem viru so dokazi itd.}
Bézierove krivulje in ploskev so polinomske, podajamo jih s pomočjo 
...
Bernsteinovi polinomi dveh spremenljivk, ki bodo definirani v poglavju \todo{ref}. 
Bézierjeve ploskve, podane v bazi prostora $\mathbb{P}_n^2$, imenujemo 
\emph{trikotne Bézierjeve ploskve}. Podrobneje bodo predstavljene v poglavju \todo{ref} 
...
povezovalne \todo{junction?} funkcije. 
...
ki morajo veljati 
za ploskve $\ddd{X}_i$ in njihove \todo{junction} funkcije v točki $\ddd{P}$. 
...
Naj bodo \todo{junction} funkcije $\alpha_i(v_i)$ konstantne, torej naj bo 
...
Naj bodo \todo{junction} funkcije $\alpha_i(v_i)$ in $\beta_i(v_i)$, $i=1,2,3$, 
...
\end{align*}
Od tod dobimo pogoje, ki morajo veljati za \todo{junction} funkcije $\alpha_i(v_i)$ 

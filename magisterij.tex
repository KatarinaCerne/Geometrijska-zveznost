...
brez škode za splošnost $n_S\geq n_R$

$$\ddd{C}(v)=\ddd{R}(0,v)=\ddd{S}(1,v)$$
\begin{align*}
  \sum_{i=0}^{n_S}\ddd{Z}_i B_i^{n_S}
\end{align*}
če $n_S=n_R$
$$\ddd{Z}_i=\ddd{Q}_{m_S,i}=\ddd{P}_{0,i}$$
sicer uporabimo formule za višanje stopnje
$$\ddd{Z}_i=\ddd{Q}_{m_S,i}=\sum_{j=0}^{n_R}\ddd{P}_{0,j}{n_R \choose j}
\frac{{n_S-n_R \choose i-j}}{{n_S \choose i}}$$

robne kontrolne točke že vnaprej določene,

zanimajo nas notranje 
...
ki pove, da to velja natanko tedaj, 
ko obstajajo polinomi $D(v)$, $E_1(v)$ 
in $F_1(v)$, kjer $$\mbox{deg}(D)\leq n_R+n_S-1$$ 
$$\mbox{deg}(E_1)\leq 2n_S-1$$
$$\mbox{deg}(F_1)\leq n_R+n_S$$
da velja $D(v)E_1(v)> 0$ na $[0,1]$ ter \eqref{eq14}
...
za $n_S=n_R$: $\mbox{deg}(E_1)=0$, $\mbox{deg}(F_1)=1$ podobno kot v primeru 
\ref{primer2}, $E_1(v)=e_0$, $F_1(v)=f_0(1-v)+f_1v$
\begin{align*}

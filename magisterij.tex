...
$\alpha_i(v_i)\equiv a_i$, funkcije $\beta_i(v_i)$ 
pa naj bodo linearni polinomi oblike 
$$\beta_i(v_i)=b_i(1-v_i)+c_iv_i.$$ 
Vpeljimo oznake $\alpha_{i,0}=\alpha_i(0)$, $\alpha_{i,1}=\alpha_i(1)$, 
$\beta_{i,0}=\beta_i(0)$ in $\beta_{i,1}=\beta_i(1)$. 
...
%\todo{??} 
Predpostavljajmo, da imamo vnaprej določene točke $\ddd{P}$ in dve izmed točk 
$\ddd{x}_{1,0}^{(i)}=\ddd{x}_{0,1}^{(i+1)}$, $i=1,\ldots,N$, na primer točki $\ddd{x}_{1,0}^{(1)}$ in 
$\ddd{x}_{1,0}^{(2)}$. Enačbe v 
\eqref{eq61} in \eqref{eq62} nam določajo preostale konstrolne točke 
...
$(\ddd{x}_{1,0}^{(1)}-\ddd{P})$ in $(\ddd{x}_{1,0}^{(2)}-\ddd{P})$. 
Ko s pomočjo enačb \eqref{eq61}, $i=2,\ldots,N-1$, izrazimo z vektorjema 
$(\ddd{x}_{1,0}^{(1)}-\ddd{P})$ in $(\ddd{x}_{1,0}^{(2)}-\ddd{P})$  
vse preostale vektorje $\ddd{x}_{1,0}^{(i)}$, $i=3,\ldots,N$, in vektor 
$\ddd{x}_{1,0}^{(N)}$ vstavmo v enačbi \eqref{eq61} za $i=1$ in \eqref{eq62}, 
dobimo pogoja oblike 
\begin{align*}
(\ddd{x}_{1,0}^{(1)}-\ddd{P})&=
...
Če enačbi \eqref{eq52} in \eqref{eq53} odvajamo še po $v_i=u_i+1$, in ju evalviramo v $v_i=u_i+1=0$, dobimo 

...
krivulj in ploskev, ima pa to slabost, da je iskanje točk na tako podani 
...
Krivulji $\ddd{p}$ in $\ddd{q}$ naj se stikata v neki točki, torej naj obstaja 
...
v skupni točki. Še en primer, kjer parametrična zveznost 
ni preveč uporabna, je t.~i. vogal kovčka. 
...
ploskvi, prikazani na slikah \ref{fig:zvezda6} in \ref{fig:zvezda6}.  

To magistrsko delo se osredotoča predvsem na konstrukcijo geometrijsko zveznih 
...
\begin{align*}
  \frac{d^k}{dt^k}\ddd{p}(t_1)=\frac{d^k}{dt^k}(\ddd{q}\circ f)(t_1),\quad k=0,1,\ldots,n.
\end{align*}
\end{definicija}
...
za $k=1,\ldots,n$, kjer je $A^k_{m_i}=\frac{k!}{i!m_1!\cdots m_i!}$. 
...
reparametrizacijska 
funkcija $f$ kot v definiciji \ref{def2}. Vpeljimo še oznako 
...
$$\frac{\partial^{k}}{\partial x^k}(x_0,y)=\frac{\partial^{k}}{\partial x^k}(\ddd{S}\circ f)(x_0,y)\quad \textrm{za } k=0,1,2,$$
...
$$\frac{\partial \ddd{R}}{\partial x}(x_0,y)=\frac{\partial}{\partial x}(\ddd{S}\circ f)(x_0,y)\quad y\in[0,1].$$
...
$$\det(\frac{\partial}{\partial x}\ddd{R}(x_0,y), \frac{\partial}{\partial u}\ddd{S}(u_0,y), 
\frac{\partial}{\partial y}\ddd{S}(u_0,y))=0.$$
...
$B_1^1(t)$. Velja $1 = B_0^1(t)+B_1^1(t)\textrm{ter}t=B_1^1(t),$
...
&=n({n-1 \choose i-1}t^{i-1}(1-t)^{n-i}-{n-1 \choose i}t^i(1-t)^{n-i-1})=\\
...
&=n\sum_{i=0}^{n-1}(\alpha_{i+1}-\alpha_i)B_i^{n-1}(t)=0.
  \end{split}
\end{equation}
Tu smo v drugi vrstici upoštevali, da je $B_n^{n-1}\equiv 0$ in 
v tretji, da je $B_{-1}^{n-1}\equiv 0$, 
...
Bazo tega prostora lahko skonsktruiramo kot tenzorski produkt baz prostorov 
...
Bézierjeve ploskve, ki jih podajamo 
...
torej velja
$\Phi(\ddd{p}_1)=\ddd{r}_{1,0,2}$, $\Phi(\ddd{p}_2)=\ddd{s}_{0,0,3}$,
$\Phi(\ddd{p}_3)=\ddd{s}_{0,1,2}$ in $\Phi(\ddd{p}_4)=\ddd{s}_{1,0,2}$. 

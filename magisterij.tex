...
V poglavju \todo{sklic} smo izpeljali, da mora v tem primeru, da dosežemo 
$G^1$-zveznost med ploskvama $\ddd{X}_i$ in $\ddd{X}_{i+1}$ 
za notranje kontrolne točke veljati zveza 
\todo{zveza}, 
na robovih pa velja 
\todo{zveza}. 
Izmed enačb \todo{sklic} nam tiste, v katerih se točka $\ddd{P}$ ne pojavi, ne 
prinesejo nobenih dodatnih pogojev, obravnavamo jih enako kot v poglavju \todo{sklic}. 
Pogoje, ki jih dobimo iz \todo{sklic}, 
...
\todo{??} 
Predpostavljajmo, da imamo vnaprej določene točke $\ddd{P}$ in \todo{točki za 1,2}. Enačbe 
\todo{sklic} nam določajo preostale konstrolne točke \todo{točke}, vse kontrolne vektorje 
\todo{vektor} lahko izrazimo kot linearno kombinacijo vektorjev \todo{vektor1} in 
\todo{vektor2}. Ko s pomočjo enačb \todo{2 do N-1} izrazimo z vektorjema \todo{vektorja} 
vse preostale vektorje \todo{vektorji} in vektor \todo{za N} vstavmi v enačbi \todo{za N in 1}, 
dobimo pogoja oblike \todo{2 pogoja}, kjer so $F_1$, $F_2$, $G_1$ in $G_2$ multilinearne funkcije 
konstant \todo{vse konstante}. Za $G^1$-zveznost v točki $\ddd{P}$ morajo zato za konstante 
\todo{vse konstante} veljati pogoji \todo{pogoji za F,G=1}. 
Oglejmo si malo bolj konkretno, kako so videti pogoji \todo{sklic} v primeru, ko je $N=3$. 
\todo{primer}

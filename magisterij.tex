...
\begin{proof}
  ...
  Dokaza se lotimo z indukcijo po $n$. \todo{vir:k}

  Naj bo najprej $n=1$. Ker je slik ploskev $\textbf{R}$ in $\textbf{S}$ $G^1$-zvezen, 
  ...
  $\ddd{R}$ in $\ddd{S}$ funkciji $\ddd{N}_r(y)$ in $\ddd{N}_s(y)$ neničelni. 
  Ker je $\ddd{N}_r(y)$ neničelna, mora biti vsaj ena izmed njenih koordinat 
  ...
  Sedaj zapišimo polinom $N_{rx}$ kot produkt dveh polinomov: 
  $N_{rx}(y)=U(y)D(y)$, kjer $U(y)$ vsebuje vse linearne delitelje polinoma 
  $N_{rx}$, $D(y)$ pa je neničeln polinom na intervalu $[0,1]$. Torej lahko pišemo 
  $$N_{sx}(y)=U(y)E_1(y)$$
  in $$W_x(y)=U(y)F_1,$$
  kjer je $E_1(y)=D(y)\alpha_1(y)$ in $F_1(y)=D(y)\beta_1(y)$.

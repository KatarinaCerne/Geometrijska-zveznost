...
$C(v)=\ddd{R}(0,v)=\ddd{S}(0,v)$. Brez škode za splošnost predpostavimo, da za stopnji 
...
\begin{align*}
\ddd{C}(v)=\sum_{i=0}^{n_S}\ddd{Z}_i B_i^{n_S}.
\end{align*}
...
stopnje 2, bomo poleg kontrolnih točk $\ddd{P}_{1,i}$ in $\ddd{Q}_{2,i}$, $i=1,2$, 
opazovali tudi kontrolne točke $\ddd{P}_{2,i}$ in $\ddd{Q}_{i,i}$, $i=1,2$. 
Poleg 
...
\begin{algorithm}[h!]
  ...
  \caption{De Casteljaujev algoritem za trikotne Bézierove krpe}
\end{algorithm}
...
$$n(\ddd{r}_{0,j+1,k}-\ddd{r}_{0,j,k+1}),\quad j+k=n-1,$$
odvod ploskve $\ddd{S}$ v smeri $\ddd{p}_3-\ddd{p}_2$ v $v_1=0$ pa 
$$n(\ddd{s}_{0,j+1,k}-\ddd{s}_{0,j,k+1}),\quad j+k=n-1.$$ 

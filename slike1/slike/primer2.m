function [Rx,Ry,Rz,Sx,Sy,Sz]=primer2(zRx,zRy,zRz,zSx,zSy,zSz)
%funkcija izraèuna manjkajoèe kontrolne toèke, tako da sta ploskvi R in S na
%skupnem robu G^1-zvezni, v primeru da sta obe ploskvi stopnje (3,3), 
%povezovalni funkciji pa sta polinoma stopnje 1
...
%kontrolna toèka P_{1,1} je doloèena s preostalimi:
Rx(2,2)=3*Rx(2,1)+(Rx(1,2)-Rx(1,1))+3*(Rx(3,2)-Rx(3,1))-(Rx(4,2)-Rx(4,1));
Ry(2,2)=3*Ry(2,1)+(Ry(1,2)-Ry(1,1))+3*(Ry(3,2)-Ry(3,1))-(Ry(4,2)-Ry(4,1));
Rz(2,2)=3*Rz(2,1)+(Rz(1,2)-Rz(1,1))+3*(Rz(3,2)-Rz(3,1))-(Rz(4,2)-Rz(4,1));
...
Sx(2,ms)=Sx(2,ms+1)-e0*px(2)-(1/3)*(e1-e0)*px(1)-(1/3)*f1*zx(1)-(2/3)*f0*zx(2);
Sy(2,ms)=Sy(2,ms+1)-e0*py(2)-(1/3)*(e1-e0)*py(1)-(1/3)*f1*zy(1)-(2/3)*f0*zy(2);
Sz(2,ms)=Sz(2,ms+1)-e0*pz(2)-(1/3)*(e1-e0)*pz(1)-(1/3)*f1*zz(1)-(2/3)*f0*zz(2);

Sx(3,ms)=Sx(3,ms+1)-e1*px(3)-(1/3)*(e0-e1)*px(4)-(2/3)*f1*zx(2)-(1/3)*f0*zx(3);
Sy(3,ms)=Sy(3,ms+1)-e1*py(3)-(1/3)*(e0-e1)*py(4)-(2/3)*f1*zy(2)-(1/3)*f0*zy(3);
Sz(3,ms)=Sz(3,ms+1)-e1*pz(3)-(1/3)*(e0-e1)*pz(4)-(2/3)*f1*zz(2)-(1/3)*f0*zz(3);
end
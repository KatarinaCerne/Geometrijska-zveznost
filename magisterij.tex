...
Bézierjeve krivulje in ploskve podajamo s pomočjo Bernsteinovih baznih polinomov, $i$-ti 
...
\begin{proof}
  ...
  za $k=1$. Enačbo \eqref{eq13} z leve vektorsko množimo z $\frac{\partial \ddd{R}}{\partial x}|_{\ddd{C}}$ 
  in dobimo 
  ...
  Če pa enačbo \eqref{eq13} z desne pomnožimo z $\frac{\partial \ddd{R}}{\partial y}|_{\ddd{C}}$, 
  dobimo 
  ...
  Iz enačbe \eqref{eq31} in definicije polinoma $E_{m+1}(y)$ je razvidno naslednje:
  \begin{align*}
  ...
  Podobno dobimo iz enačbe \eqref{eq32} in definicije polinoma $F_{m+1}(y)$ oceno za 
...
Parametri so določena z enačbama \eqref{eq18} in \eqref{eq19} na naslednji način. 
...
Enačba \eqref{eq15} se torej v tem primeru preoblikuje tako:
\begin{align}\label{eq16}
...
Sklicujmo se na enakost \eqref{eq28} in dobimo naslednje rezultate za parcialne odvode 
...
le dva še neznana parametra $e_{20}$ in $f_{20}$, ki ju lahko določimo iz enačbe. %\todo{skalarni produkt z normalo?} V tem primeru nam enačba 
...
Enako lahko s pomočjo enačb za višanje stopnje izrazimo kontrolne vektorje 
...
Trikotne Bézierjeve ploskve so tip Bézierovih ploskev s parametrizacijo, definirano 
